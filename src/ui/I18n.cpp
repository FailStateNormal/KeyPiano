...
Translator::Translator(QObject* parent) : QTranslator(parent) {
    ...
    map_ = {
        ...
        {"Export to &WAV...", QStringLiteral("导出为 &WAV...")},
        {"Controls", QStringLiteral("控制")},
        {"Audio", QStringLiteral("音频")},
        {"Volume", QStringLiteral("音量")},
        {"Background Play (map keys when not focused)",
         QStringLiteral("后台演奏（不聚焦时也映射按键）")},
        {"MIDI Input", QStringLiteral("MIDI 输入")},
        {"(None)", QStringLiteral("（无）")},
        {"MIDI input disabled.", QStringLiteral("已关闭 MIDI 输入。")},
        {"MIDI input: %1", QStringLiteral("MIDI 输入：%1")},
        {"Could not open MIDI device: %1",
         QStringLiteral("无法打开 MIDI 设备：%1")},
        ...
        {"Open Recording", QStringLiteral("打开录音")},
        {"Open Error", QStringLiteral("打开错误")},
        {"Failed to open recording:\n%1",
         QStringLiteral("打开录音失败：\n%1")},
        {"Loaded recording: %1 (%2 events)",
         QStringLiteral("已加载录音：%1（%2 个事件）")},
        ...
        {"Export to WAV", QStringLiteral("导出为 WAV")},
        {"Nothing to export.\nRecord something first.",
         QStringLiteral("没有可导出的内容。\n请先录制一段。")},
        {"WAV audio (*.wav);;All files (*)",
         QStringLiteral("WAV 音频 (*.wav);;所有文件 (*)")},
        {"No SoundFont is available to render the export.",
         QStringLiteral("没有可用于渲染导出的音色库。")},
        {"Export failed:\n%1", QStringLiteral("导出失败：\n%1")},
        {"Exported: %1", QStringLiteral("已导出：%1")},
        {" (rendered with the SF2 piano, not the VST3 plugin)",
         QStringLiteral("（使用 SF2 钢琴音色渲染，而非 VST3 插件）")},
